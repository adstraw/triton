...
void init_gluon_ir(py::module &&m) {
  ...
  py::class_<GluonOpBuilder, TritonOpBuilder>(
     ...
     .def("create_tcgen05_scaled_mma",
          [](GluonOpBuilder &self, Value a, Value aScale, tt::ScaleDotElemType aType,
             Value b, Value bScale, tt::ScaleDotElemType bType,
             Value acc, Value useAcc,
             Value pred, std::vector<Value> &mbarriers,
             std::vector<Value> &mbarrier_preds) {
            Value accDep;
            auto tokType = self.getBuilder().getType<ttg::AsyncTokenType>();
            self.create<ttng::TCGen5MMAScaledOp>(tokType, a, b, acc, accDep,
                                           aScale, bScale, aType, bType, useAcc,
                                           pred, mbarriers,
                                           mbarrier_preds);
          })
